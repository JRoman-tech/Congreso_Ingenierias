...
import ValidacionDetalle from './pages/ValidacionDetalle'
import MiValidacion from './pages/MiValidacion'
...
function AdminRoutes({ path }: { path: string }) {
  ...
  const validation = path.match(/^\/dashboard\/validacion\/(\d+)$/)
  ...
  if (validation) return <ValidacionDetalle id={Number(validation[1])} />
...
function ParticipantRoutes({
  ...
  if (path === '/dashboard/trabajos') {
    return <Trabajos participantId={participantId} />
  }
  if (path === '/dashboard/validacion') {
    return <MiValidacion participantId={participantId} />
  }
  ...
  if (work) {
    ...
    return (
      <TrabajoForm
        id={work[1]}
        readOnly={readOnly}
        fixedParticipantId={participantId}
      />
    )
  }
...
function ApplicationRoutes() {
  ...
  const page = user.rol === 'administrador'
    ...
    : (
      <ParticipantRoutes
        path={path}
        participantId={user.participante_id ?? ''}
      />
    )
